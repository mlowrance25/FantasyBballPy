...
def getTeamNames(writeToDb):
    teamNames = NbaClient.GetAllNbaTeamNames()
    ...
    for team in sorted(teamDict.keys()):
        ...
        if writeToDb:
            database.AddTeamToDb(nbaTeamId,team,tricCode)
    return teamDict
...
def getTodaysSchedule(teamDict):
    ...
    for game in schedule['league']['standard']:
        ...
        if nbaHomeTeamId not in teamDict.key() or nbaVisitorTeamId not in teamDict.keys():
            continue
        if startTeam > '2018-12-02T08' and startTime < '2018-12-03T08':
            yesterdayTeams.append(nbaHomeTeamId)
            yesterdayTeams.append(nbaVisitorTeamId)
        if startTeam > '2018-12-03T08' and startTime < '2018-12-04T08':
...
def getAllPlayers(teamDict):
    playerDict = dict()
    allPlayers = NbaClient.GetLeagueRoster()
    playerIds = []
    for player in allPlayers['league']['standard']:
        ...
        if nbaTeamId:
            ...
            playerDic[nbaTeamId].append(dict(PlayerID=nbaPlayerId,FirstName=fName,LastName=lName,PostionID=position,Stats=[]))
    ...
    if nbaTeamId != '' and int(nbaTeamId) in NbaTeamIdToDbIdEnum.keys():
        ...
        for possiblePlayer in teamRoster:
            ...
            if tempFname == fName and tempLName == lName:
                playerFound = True
                newPosition = possiblePlayer[2].strip(",").replace('  ','').replace("'",'').replace('.','').lower()
...
def GetPlayerStats(teamIds,playerDict,playerIds,teamsPlayingToday):
    ...
    for game in leagueSchedule['league']['standard']:
        ...
        if homeTeamId not in teamsPlayingToday and visitorTeamId not in teamsPlayerToday:
            continue
        else:
            if dateString > '20181016' and dateString < '20181203':
                ...
                for row in statsForGame['stats']['activePlayers']:
                    ...
                    if nbaTeamId != '' and homeTeamId in teamIds and visitorTeam in teamIds:
                        ...
                        if gameInDb:
                            ...
                            if currentNbaPlayerID in playerIds:
                                statDict = dict(Date=dateString,GameID=nbaGameId,OpposingTeamID=opposingTeamID,PointsScored=ptsScored,ThreesMade=threePointersMade,Rebounds=rebounds,Assists=assists,Steals=steals,Turnovers=turnovers,Blocks=blocks)
